# AGENTS.md - Vue3 + TypeScript + Vite Project
...
This project currently has no test framework installed. To add testing:
...
- Use absolute imports with `@/` alias (configure in vite.config.ts if needed)
- Group imports: Vue/core → third-party → local (relative imports)
- Use TypeScript file extensions for TS files (`.ts`, `.vue`)
- Avoid importing from `vue` internal paths
...
// Avoid
import { ref } from 'vue/src/reactivity/ref'  // Don't import internals
```
...
- Use `type` instead of `interface` for object types (per Vue3 style guide preference)
...
// Avoid
function getData(x: any) {  // Don't use any
  return x
}
```
...
- **Components**: PascalCase (e.g., `MyComponent.vue`, `UserProfile.tsx`)
- **Composables**: camelCase with `use` prefix (e.g., `useCounter.ts`, `useUser.ts`)
- **Types/Interfaces**: PascalCase (e.g., `User`, `ApiResponse`)
- **Variables/Functions**: camelCase (e.g., `userName`, `getUserInfo()`)
- **Constants**: UPPER_SNAKE_CASE (e.g., `MAX_RETRIES`, `API_BASE_URL`)
- **CSS Classes**: kebab-case (e.g., `.user-profile`, `.main-container`)
...
- Top-level component structure: script → template → style (scoped preferred)
...
<template>
  <div class="my-component">
    <h1>{{ title }}</h1>
    <button @click="emit('click', localCount++)">
      Clicked {{ localCount }} times
    </button>
  </div>
</template>
...
- Log errors with context (avoid console.log in production)
...
├── views/            # Page components (if using Vue Router)
├── router/           # Vue Router config (if used)
